...
def generate_shuffled_controls(standard_prompts, tokenizer):
    """
    STRONG CONTROL: Shuffles the exact tokens from the Level 5 prompts.
    This proves that SYNTAX (order), not VOCABULARY (complexity), causes the wall.
    """
    controls = []
    # Filter for the most complex prompts (Level 5)
    targets = [p for p in standard_prompts if "Level 5" in p['label']]
    
    print(f"Generating {len(targets)} shuffled control samples...")
    
    for item in targets:
        # 1. Get the actual tokens from the logical prompt
        input_ids = tokenizer(item['prompt'], return_tensors='pt')['input_ids'][0].tolist()
        
        # 2. Shuffle the order of these specific logical tokens
        random.shuffle(input_ids)
        
        # 3. Decode back to text (this creates "word salad" using high-level concepts)
        shuffled_text = tokenizer.decode(input_ids, skip_special_tokens=True)
        
        controls.append({"label": "Control (Shuffled)", "prompt": shuffled_text})
...
if __name__ == "__main__":
    ...
    control_cases = generate_shuffled_controls(experiment_cases, topo.tokenizer)
    ...
    # Check the stats for the "Associative" group (where Curvature is flat/high)
    associative_data = df_exp[df_exp['Kappa'] > -1.5] 
